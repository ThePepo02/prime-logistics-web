<template>
    <div class="p-6 bg-gray-50 min-h-screen">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <button
                class="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-100">
                ⬇ Exportar
            </button>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200">
            ...
            <div class="flex gap-3 p-4 border-b border-gray-100">
                <input v-model="filtros.cerca" @input="cargarOperacions(1)" type="text"
                ...
                <select v-model="filtros.modo" @change="cargarOperacions(1)"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <tr v-else v-for="op in operacions" :key="op.id"
            ...
            <div class="flex justify-between items-center px-5 py-3 border-t border-gray-100 text-sm text-gray-500">
                ...
                <div class="flex gap-1">
                    <button v-for="p in paginacion.last_page" :key="p" @click="cargarOperacions(p)"
...
const stats = ref({ total: 0, aerea: 0, maritima: 0, pct_aerea: 0, pct_maritima: 0 })
const distribucio = ref([])
const operacions = ref([])
const paginacion = ref({ total: 0, current_page: 1, last_page: 1 })
const filtros = ref({ cerca: '', modo: '' })
...
const cargarDistribucio = async () => {
    const res = await fetch('/api/operaciones/distribucio', {
        headers: { Authorization: `Bearer ${token()}` }
    })
    distribucio.value = await res.json()
}

const cargarOperacions = async (page) => {
    cargando.value = true
    const params = new URLSearchParams({
        page,
        ...(filtros.value.cerca && { cerca: filtros.value.cerca }),
        ...(filtros.value.modo && { modo: filtros.value.modo }),
    })
    const res = await fetch(`/api/operaciones/operacions?${params}`, {
        headers: { Authorization: `Bearer ${token()}` }
    })
    const json = await res.json()
    operacions.value = json.data
    paginacion.value = json
