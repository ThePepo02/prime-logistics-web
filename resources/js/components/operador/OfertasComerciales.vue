  ...
  <div class="p-6">
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 mb-4">
      <div class="flex flex-wrap gap-3">
        ...
        />
        <select v-model="filtreEstat" class="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400">
          <option value="">Estado</option>
          <option v-for="estat in estats" :key="estat.id" :value="estat.id">{{ estat.estat }}</option>
        </select>
        <select v-model="filtreTransport" class="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400">
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr v-for="oferta in ofertasFiltrades" :key="oferta.id" class="hover:bg-gray-50 transition">
            <td class="px-4 py-3 font-semibold text-gray-800">OC-{{ String(oferta.id).padStart(4, '0') }}</td>
            <td class="px-4 py-3 text-gray-700">{{ oferta.client ? oferta.client.nom + ' ' + oferta.client.cognoms : '—' }}</td>
            <td class="px-4 py-3 text-orange-500 font-medium">{{ oferta.client ? oferta.client.empresa : '—' }}</td>
            <td class="px-4 py-3">
      ...
      <div class="flex items-center justify-between px-4 py-3 border-t border-gray-100 text-sm text-gray-500">
        ...
        <div class="flex gap-1">
          <button
            v-for="page in ofertes.last_page" :key="page"
            @click="carregarOfertes(page)"
